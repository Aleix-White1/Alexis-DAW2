...
@app.route('/BioHistorial')
def BioHistorial():
    return render_template('BioHistorial.html')
...
def parteBio(comandoBio):
    if  "buscar proteínas por ID" in comandoBio:
        IDs = comandoBio[::24]
        busqueda = Query(IDs, "PubmedIdQuery").search()
        return busqueda
...
def menuComandos(comando):
    ...
    elif "menú" in comando or "que sabes hacer" in comando or "qué haces" in comando:
        Historial(comando)
